...
def gen_qdates(start_date: datetime, end_date: datetime, freq: str = "12H") -> list:
    ...
    if len(final_list) == 0:
        start_date_utc = start_date.strftime(fmt)
...
def del_curr(**kwargs):
    ...
    print("Descargando datos del día hasta {0}".format(data_interval_end.astimezone(cl)))
    data_interval_start = data_interval_end.replace(hour=0, minute=0, second=0, microsecond=0)
    ...
    update_job = client.query(
        ...
        DELETE FROM `{0}` 
        WHERE start_date >= "{1}"
        """.format(bq_table, data_interval_start))

    print(update_job.result())

def descargar_consumo(**kwargs):
    """Descarga de datos desde API de Mediastream y conversión a formato adecuado
    """
    #print(kwargs["dag_run"].conf["update"])
    ...
    # Cálculo de rangos de fechas para consultas
    qdates = gen_qdates(data_interval_start, data_interval_end)
...
with DAG(
    dag_id='consumo_detalle_hora',
    start_date=datetime(2022, 9, 10, tzinfo=local_tz),
    catchup=False,
    tags=["audio","diario","live","on demand","platform"],
) as dag:
